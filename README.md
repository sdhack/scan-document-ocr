...
在同一台机器、同一OCR模型、同一PDF页面和相同渲染参数下，分别强制使用GPU与CPU进行单页对照测试：

| 测试 | GPU | CPU | GPU相对速度 |
|---|---:|---:|---:|
| 同页测试A | 0.221秒/页 | 1.199秒/页 | 约5.4倍 |
| 同页测试B | 0.165秒/页 | 1.073秒/页 | 约6.5倍 |

按测试A的单页速度粗略换算，374页纯文字OCR推理约为：

- GPU：约83秒；
- CPU：约448秒（约7分28秒）。

这个换算只比较文字OCR推理，不包括PDF渲染、表格识别、插图裁剪、磁盘写入和模型初始化。《中国居民膳食指南（2022年版）》完整执行了正文、表格和插图流程，因此总耗时约3分14秒，不能直接与纯文字单页速度等同。

GPU的主要价值是缩短推理时间并减少CPU承担OCR计算的压力，不是自动提高识别准确率。同一模型的最终效果仍受扫描清晰度、页面倾斜、字体、版式、渲染倍率和模型版本影响。上述5.4—6.5倍是本机实测结果，不代表所有GPU、CPU和PDF都能达到相同比例。
